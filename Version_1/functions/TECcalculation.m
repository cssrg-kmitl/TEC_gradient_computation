function [ROTI,prm,psudo] = TECcalculation(obs,nav,satb,S_path)
...
psudo.c1_p           = nan(86400,32);
psudo.l1_p           = nan(86400,32);
psudo.p2_p           = nan(86400,32);
psudo.l2_p           = nan(86400,32);
TECrel               = nan(86400,32);
...
prm.azi              = nan(86400,32); % azimuth angle
...
for i = 1 : length(Sat_obs) % GPS 1 - 32
    try
        ...
        Xl = RL(:,1);
        Yl = RL(:,2);
        prm.azi(Time+1,PRN) = atan2(Xl,Yl)*180/pi;
        
        % 2.3 Calculate STEC
        %===== STEC Pseudorange
        psudo.c1_p(Time+1,PRN) = C1;
        psudo.l1_p(Time+1,PRN) = L1;
        psudo.p2_p(Time+1,PRN) = P2;
        psudo.l2_p(Time+1,PRN) = L2;
        STECp(Time+1,PRN) = k*(P2-C1);
        %===== STEC Carrier phase
        STECl(Time+1,PRN) = k*(L1-L2);
        TECrel(Time+1,PRN)= ((C1-L1)*(f1.^2))/(2*40.3*10^16);
...
% STECl_M_new = CycleSlipCorrection_v2(TEC.STECl,prm.elevation,prm.Timces,Sat_obs');
% STECl_M_new = TEC.STECl;
% 2.6 Adjusted STEC
...
prm.TEC_slant = TEC.slant;
prm.TEC_vertical = TEC.vertical;
...
name5 = ['psudo_' year '_' month '_' date];
eval([name1 '= TEC;'])
eval([name2 '= DCB;'])
eval([name3 '= ROTI;'])
eval([name4 '= prm;'])
